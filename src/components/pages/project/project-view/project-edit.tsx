...
function ProjectEdit({
  ...
  const form = useForm<z.infer<typeof formSchema>>({
    ...
    defaultValues: {
      ...
      logo: undefined,
      repository_url:
        project.urls && project.urls[0]?.repository_url
          ? project.urls[0].repository_url
  ...
  function onSubmit(values: z.infer<typeof formSchema>) {
    try {
      ...
      axios
        .post(`/api/projects/${project.id}`, formData, {
          headers: {
        ...
        .catch((e) => {
          toast.error(
            "Произошла ошибка при обновлении проекта, попробуйте позже",
          );
  ...
  function removeUser(userId: number) {
    axios
      .delete(`/api/projects/${project.id}/users/${userId}`)
      .then(() => {
      ...
      .catch((error) => {
        toast.error(
          "Не удалось удалить пользователя из проекта, попробуйте позже: " +
  ...
  function handleUserRoleChange(userId: number, role: string) {
    axios
      .put(`/api/projects/${project.id}/users/${userId}`, { role })
      .then(() => {
      ...
      .catch((error: any) => {
        toast.error(
          "Не удалось обновить роль пользователя, попробуйте позже" +
  ...
  function handleProjectDeletion() {
    axios
      ...
      .catch((error) => {
        toast.error(
          "Произошла ошибка при удалении проекта, попробуйте позже" +
            error.message,
        );
      });
  }
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  ...
                  <FormDescription>
                    Краткое описание позволяющее понять ваш проект
                  </FormDescription>
                  <FormMessage />
                </FormItem>
              )}
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  ...
                  <FormDescription>
                    Выберите изображение для обложки вашего проекта.
                  </FormDescription>
                  <FormMessage />
                </FormItem>
              )}
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  ...
                  <FormDescription>
                    Выберите изображение для логотипа вашего проекта.
                  </FormDescription>
                  <FormMessage />
                </FormItem>
              )}
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  ...
                  <FormDescription>
                    Укажите URL вашего репозитория (например, GitHub, GitLab).
                  </FormDescription>
                  <FormMessage />
                </FormItem>
              )}
            ...
            {projectUsers &&
              projectUsers.map((user) => (
                ...
                >
                  ...
                  <Select
                    value={user.pivot ? user.pivot.role : "member"}
                    onValueChange={(value) => {
