...
import {
  IProject,
  IProjectUser,
} from "@/components/widgets/projects/types/ProjectTypes";
...
import UserPopover from "@/components/ui/userpopover.tsx";
  ...
  useEffect(() => {
    if (user) {
      ...
      setIsEditable(
        projectUsers.some(
          (u) =>
            u.id === user.data.id && ["admin", "owner"].includes(u.pivot.role),
        ) ||
  ...
  return (
    <div className="flex flex-col mx-10 py-6 justify-center">
      <div className="">
        {project.cover ? (
          <img
            src={`${import.meta.env.VITE_APP_URL}/storage/${project.cover}`}
            alt="Cover preview"
        ...
        <Tabs defaultValue="overview">
          ...
          <TabsContent value={"overview"}>
            <div className="flex flex-wrap space-y-2 md:flex-row space-x-4 px-8 py-4 md:justify-between">
              {project.logo && (
                <img
                  src={`${import.meta.env.VITE_APP_URL}/storage/${project.logo}`}
                  alt="Logo preview"
              ...
              <div className="flex flex-col space-y-2 order-2 md:order-1">
                <div className="flex flex-row flex-wrap space-y-2 items-center space-x-4">
                  <p className="text-4xl font-bold">{project.name}</p>
                  <Badge className="w-24 h-8" variant={"outline"}>
                    {project.privacy === "public" ? "Публичный" : "Приватный"}
                  </Badge>
                </div>
                <div>
                  <p className="text-secondary-foreground whitespace-normal break-all">
                    {project.description}
                  </p>
                </div>
                ...
                <div className="flex space-x-1">
                  {project.urls?.map((link, index) => (
                    <Badge key={`repo-${index}`} className="bg-white outline-1">
                      <GitBranch className="invert" />
                      <a
                        href={link.repository_url}
                        target="_blank"
              ...
              <div className="max-w-[80%] space-y-2 flex-grow order-2 md:order-1 px-4 py-2">
                {project.annotation && (
                  <div className={"inline-flex flex-col gap-1"}>
                    <p className={"flex gap-1 flex-row"}>
                      <FileText />
                      <span className={"text-xl font-medium"}>Аннотация: </span>
                    </p>
                    <p className={"border px-2 py-1 rounded-md text-wrap"}>
                      {project.annotation}
                    </p>
                  </div>
                )}
                <div className="inline-flex gap-1 items-center text-xl font-medium">
                  ...
                  {isEditable && (
                    <Button
                      variant="ghost"
                      size="sm"
                      onClick={() => setIsReadmeEditorOpen(true)}
                      className="ml-2"
                    >
                ...
                <div
                  className={
                    "text-wrap border p-2 rounded min-h-32 prose dark:prose-invert max-w-none overflow-auto"
                  }
                  dangerouslySetInnerHTML={{ __html: readmeContent }}
                />
              </div>
              <div className="flex order-1 flex-grow-0 md:order-2 flex-col max-w-96 px-2 mx-3 md:mx-0 py-1 rounded-md border-1 border-gray-200">
                ...
                <div className="flex flex-col py-1.5 space-y-1">
                  {project.users?.length ? (
                    ...
                    <div className={"text-center text-muted-foreground w-36"}>
                      Нет участников
                    </div>
                  )}
                </div>
          ...
          <TabsContent value="files">
            <ProjectFiles
              isProjectUser={isProjectUser || isEditable}
              project={project}
            />
          </TabsContent>
        ...
        <Dialog open={isReadmeEditorOpen} onOpenChange={setIsReadmeEditorOpen}>
          <DialogContent className="sm:max-w-[80vw] md:max-w-[70vw] lg:max-w-[60vw] w-full">
            ...
            <div className="flex flex-col h-full overflow-hidden">
              ...
              <div className="prose dark:prose-invert max-w-none flex-grow overflow-auto border rounded-md p-2">
            ...
            <DialogFooter className="mt-auto pt-4">
              <DialogClose asChild>
                <Button
                  type="button"
                  variant="secondary"
                  onClick={() => setIsReadmeEditorOpen(false)}
                >
                  Отмена
                </Button>
              </DialogClose>
              <Button
                ...
                onClick={() => {
                  ...
                  axios
                    .put(`/api/projects/${id}/readme`, { readme: newReadme })
                    .then((res) => {
                      if (res.status === 200) {
                        setReadmeContent(newReadme);
                    ...
                    .catch((err) => {
                      toast.error(
                        "Ошибка при сохранении README: " + err.message,
                      );
                    })
                    .finally(() => {
