...
export function UserAutocomplete({
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (response.status === 200) {
        ...
        console.error("Failed to submit users")
      }
    } catch (error) {
  ...
  return (
    <div className={cn("relative w-full", className)}>
      <div className="flex flex-wrap items-center gap-1 p-2 border rounded-md focus-within:ring-2 focus-within:ring-ring focus-within:ring-offset-2">
        ...
        <div className="relative flex-1 min-w-[120px]">
          <div className="absolute left-0 flex items-center h-full pointer-events-none">
            {query === "" && selectedUsers.length === 0 && <Search className="w-4 h-4 text-muted-foreground" />}
          </div>
          <input
            ref={inputRef}
            type="text"
