...
function UserBadge({ className, ...props }: UserBadgeProps) {
  return (
    <div
      className={cn(
        "inline-flex justify-center text-s font-medium w-fit px-2 py-1 items-center gap-1.5",
        className,
      )}
    >
      <Avatar>
        {props.user.avatar ? (
          <AvatarImage
            src={`${import.meta.env.VITE_APP_URL}/storage/${props.user.avatar}`}
            alt="Avatar"
          />
        ) : (
      ...
      {props.portable ? (
        ...
        <span className="font-light font-montserrat">
          {props.withFullName
            ? `${props.user.surname} ${props.user.firstname.slice(0, 1)}. ${props.user.middlename ? props.user.middlename.slice(0, 1) + "." : ""} (@${props.user.name})`
            : `@${props.user.name}`}
        </span>
      )}
    </div>
