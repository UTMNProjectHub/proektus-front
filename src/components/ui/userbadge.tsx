...
function UserBadge({className, ...props}: UserBadgeProps) {
  return (
    <div className={cn("inline-flex justify-center text-s font-medium w-fit px-2 py-1 items-center gap-1.5", className)}>
      <Avatar className="shrink">
        {props.user.avatar ? (<AvatarImage src={`${import.meta.env.VITE_APP_URL}/storage/${props.user.avatar}`} alt="Avatar" />) : (<AvatarFallback className="text-black">{props.user.firstname.slice(0, 1)}</AvatarFallback>)}
      </Avatar>
